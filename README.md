...
A native DSH bundle lives in [`.dsh-plugin/`](.dsh-plugin/): it registers the six `/ponytail*` slash commands and serves the six skills through the skill registry's global layer (so they reach every agent/preset, including the web profile whose host skill-filesystem row is disabled).
...
Restart `dsh web` after installing. Once the package is published to npm as `dsh-plugin-ponytail`, the same command takes the package name instead of a path. The bundled `skills/` copies are byte-checked against this repo's `skills/` by `tests/dsh-plugin.test.js`; refresh them with `node .dsh-plugin/scripts/sync-skills.mjs`.
