...
| Muse Code | `.agents/skills/`, `AGENTS.md` | Project skills from `.agents/skills/<id>/SKILL.md` when workspace is trusted (`muse --trust-workspace`); `AGENTS.md` at repo root is always-on rules. Six skills: `ponytail` (lite/full/ultra), `ponytail-review`, `ponytail-audit`, `ponytail-debt`, `ponytail-gain`, `ponytail-help`. Validate with `muse skills validate .agents/skills/ponytail --json`. |
