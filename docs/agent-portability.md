...
| DeepSeek Harness (DSH) | `.dsh-plugin/` (`cordis.patch.yml`, `lib/`, `skills/`) | Native DSH bundle: one cordis patch mounts a zero-dependency Node plugin that registers the six `/ponytail*` commands and a global-layer skill provider serving `.dsh-plugin/skills/` (the web profile disables the host skill-filesystem row; global providers reach every preset). Skills are byte-checked copies of `skills/`, refreshed by `scripts/sync-skills.mjs`. |
