...
| DeepSeek Harness (DSH) | External [`dsh-ponytail`](https://github.com/NOirBRight/dsh-ponytail) bundle | Install `github:NOirBRight/dsh-ponytail#v0.2.0` with `dsh plugin`; the `dsh-ponytail-skills` package registers the six skills, session-backed modes, dynamic prompt policy, commands, subagent inheritance, and responsive Web/mobile settings without modifying DSH Core. |
