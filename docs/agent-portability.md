...
| AtomCode | `.claude-plugin/`, `hooks/claude-codex-hooks.json`, `skills/` | Reuses the Claude Code plugin protocol: AtomCode reads `.claude-plugin/marketplace.json` and `.claude-plugin/plugin.json` (whose `hooks` field points at `hooks/claude-codex-hooks.json`), and auto-scans `skills/`. `/plugin marketplace add https://github.com/DietrichGebert/ponytail` + `/plugin install ponytail@ponytail`; the six skills land in the `/` menu namespaced `ponytail:`. The Claude-specific `commands/*.toml` files are silently skipped, the skills cover the same invocations. Hooks are inert until `atomcode plugin trust ponytail`, then load next session; the lifecycle hooks (activation + mode tracking) run as-is. |
