...
| Kimi Code | `kimi.plugin.json`, `hooks/kimi-code-hooks.toml`, `hooks/`, `skills/`, `AGENTS.md` | Kimi Code 0.40+: `/plugins install https://github.com/DietrichGebert/ponytail` — the plugin manifest registers the six `skills/` as commands and a `UserPromptSubmit` hook that activates the default mode, tracks `/ponytail` switches, and injects the ruleset every turn (a hook's stdout is appended to the model context on exit 0). Plugin hooks run with `KIMI_CODE_HOME`/`KIMI_PLUGIN_ROOT` set and cwd at the plugin root. Subagent injection is unavailable in 0.38–0.40.1 (`PreToolUse` stdout discarded, `SubagentStart` observation-only); the manifest registers the `PreToolUse`/`Agent` hook anyway so it unlocks when that changes. Pre-0.40 fallback: merge the `[[hooks]]` block from `hooks/kimi-code-hooks.toml` into `~/.kimi-code/config.toml` and copy `skills/` into `~/.kimi-code/skills/`. Reads `AGENTS.md` from the project root as instruction-tier fallback. |
